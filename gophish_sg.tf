...
# Allow ingress from Teamserver instances via port 22 (SSH) for Ansible
# configuration and port 587 (SMTP mail submission) so mail can be sent via the
# mail server on Gophish instances
resource "aws_security_group_rule" "ingress_from_teamserver_to_gophish_via_ssh_and_smtp" {
