...
data "cloudinit_config" "terraformer_cloud_init_tasks" {
  ...
  # Create a file /var/terraformer/aws_config that can be used to
  # configure the AWS CLI to assume the Terraformer role by default.
  # This file can be utilized by a user by simply copying it to
  # ~/.aws/config.  For details, see
  ...
  part {
    content = templatefile(
      "${path.module}/cloud-init/write-aws-config.tpl.yml", {
        ...
        owner       = "root:root"
        path        = "/var/terraformer/aws_config"
        permissions = "0444"
