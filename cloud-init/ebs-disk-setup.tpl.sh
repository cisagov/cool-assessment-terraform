...
# This script is used to prepare an EBS volume for use by creating a file
# system (if necessary), mounting it, and ensuring that it will remount
# after a reboot.
...
uuid=$(blkid --match-tag UUID --output value "${device_name}")

# Make the mount point directory (if it doesn't already exist)
